...
export default function TADashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'classes' | 'content'>('classes');
  ...
  const fetchDashboardData = async () => {
    try {
      const [classesData, contentData] = await Promise.all([
        api.getProfessorClasses(), // TAs use same endpoint
        api.getProfessorContent(),  // TAs use same endpoint
      ]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center">
        <div className="text-neutral-600">Loading TA dashboard...</div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-warning-50 via-white to-neutral-50">
      {/* Header */}
      <header className="bg-white border-b border-neutral-200 shadow-soft">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-neutral-900">Teaching Assistant Dashboard</h1>
              <p className="text-sm text-neutral-600">Manage course content and assist professors</p>
            </div>
            <button
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold text-neutral-900">
                    Classes I'm Assisting ({classes.length})
                  </h2>
                </div>
                ...
                {classes.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-neutral-500 mt-2">
                      Professors will assign you to classes
                    </p>
                  </div>
                ) : (
                    ...
                    {classes.map((cls) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <span className="badge badge-warning">{cls.class_code}</span>
                        </div>
              ...
              <div>
                ...
                {content.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-neutral-500 mt-2">
                      Create lectures, assignments, and materials
                    </p>
                  </div>
                ) : (
